drop policy "Admins can insert status history" on "public"."order_status_history";

drop policy "Users can view order status history" on "public"."order_status_history";

drop policy "Admins can update orders" on "public"."orders";

drop policy "Admins can view review queue" on "public"."payment_manual_review_queue";

drop policy "Allow admin insert access" on "public"."products";

drop policy "Allow admin update access" on "public"."products";

drop policy "Allow public read access" on "public"."products";

drop policy "Profiles updatable by owners and admins" on "public"."profiles";

drop policy "Viewable by admins or public if active" on "public"."promo_codes";

drop policy "Admin update access" on "public"."site_settings";

drop policy "Public read access" on "public"."site_settings";

drop policy "Admins can perform all actions on tickets" on "public"."support_tickets";
...
drop policy "support_tickets_admin_update" on "public"."support_tickets";

drop policy "Users read own payment intents" on "public"."payment_intents";

drop policy "Authenticated users can insert promo code attempts" on "public"."promo_code_attempts";

alter table "public"."email_template_variables" drop constraint "email_template_variables_insertion_order_check";

alter table "public"."email_variable_registry" drop constraint "email_variable_registry_key_format";

alter table "public"."email_variable_registry" drop constraint "email_variable_registry_render_policy_check";

alter table "public"."email_variable_registry" drop constraint "email_variable_registry_value_type_check";

alter table "public"."email_template_variables" drop constraint "email_template_variables_variable_key_fkey";

alter table "public"."orders" drop constraint "orders_currency_tl_only";

drop function if exists "public"."deduct_stock_on_order"();

drop function if exists "public"."manage_stock_on_status_change"();

alter table "public"."email_variable_registry" drop constraint "email_variable_registry_pkey";
...
alter table "public"."email_variable_registry" add column "category" text default 'general'::text;

alter table "public"."email_variable_registry" add column "id" uuid not null default gen_random_uuid();
...
alter table "public"."order_status_history" alter column "status" set data type character varying(50) using "status"::character varying(50);
...
alter table "public"."site_settings" drop column "created_at";

CREATE UNIQUE INDEX email_variable_registry_key_key ON public.email_variable_registry USING btree (key);

CREATE UNIQUE INDEX email_variable_registry_pkey ON public.email_variable_registry USING btree (id);

alter table "public"."email_variable_registry" add constraint "email_variable_registry_pkey" PRIMARY KEY using index "email_variable_registry_pkey";

alter table "public"."email_variable_registry" add constraint "email_variable_registry_key_key" UNIQUE using index "email_variable_registry_key_key";

alter table "public"."email_template_variables" add constraint "email_template_variables_variable_key_fkey" FOREIGN KEY (variable_key) REFERENCES public.email_variable_registry(key) ON DELETE CASCADE not valid;

alter table "public"."email_template_variables" validate constraint "email_template_variables_variable_key_fkey";
