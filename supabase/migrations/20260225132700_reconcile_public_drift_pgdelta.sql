...
END;
$function$;
ALTER FUNCTION public.set_payment_updated_at SET search_path TO public;
COMMENT ON COLUMN public.orders.shipping_address_id IS 'Reference to delivery address';
COMMENT ON COLUMN public.orders.status IS 'pending, processing, shipped, delivered, cancelled';
COMMENT ON COLUMN public.orders.payment_method IS 'credit_card or bank_transfer';
