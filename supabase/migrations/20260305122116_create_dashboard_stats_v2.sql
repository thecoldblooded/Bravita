...
BEGIN
    ...
    SELECT COALESCE(SUM((order_details->>'total')::numeric), 0), COUNT(*)
    INTO v_total_revenue, v_order_count
    FROM orders
    WHERE created_at >= start_date AND created_at <= end_date AND status != 'cancelled';

    SELECT COUNT(*) INTO v_cancelled_count
    FROM orders
    WHERE created_at >= start_date AND created_at <= end_date AND status = 'cancelled';

    SELECT COUNT(*) INTO v_new_member_count
    FROM profiles
    WHERE created_at >= start_date AND created_at <= end_date;

    SELECT COUNT(*) INTO v_active_product_count
    FROM products
    ...
    FROM (
        ...
        FROM orders, jsonb_array_elements(order_details->'items') as item
        WHERE created_at >= start_date AND created_at <= end_date AND status != 'cancelled'
    ...
    FROM (
        SELECT o.id, p.full_name, (o.order_details->>'total')::numeric as total,
