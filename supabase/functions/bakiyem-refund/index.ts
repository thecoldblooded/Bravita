...
serve(async (req: Request) => {
    if (req.method === "OPTIONS") {
        ...
        if (origin && !isAllowedOrigin(origin)) {
            return new Response(JSON.stringify({ success: false, pending: false, message: "Forbidden origin" }), {
                status: 403,
                headers: corsHeaders(req),
            });
        }
    ...
    const origin = req.headers.get("Origin") ?? "";
    if (origin && !isAllowedOrigin(origin)) {
        return response(req, 403, { success: false, pending: false, message: "Forbidden origin" });
    }
