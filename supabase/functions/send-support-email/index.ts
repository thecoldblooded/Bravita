...
function getCorsHeaders(req: Request) {
    ...
    return {
        ...
        "Access-Control-Allow-Headers": "authorization, x-client-info, apikey, content-type, x-bravita-secret, x-user-jwt",
...
serve(async (req: Request) => {
    ...
    try {
        ...
        // 3. CAPTCHA validation only for guest ticket creation flow (Bypassed since create-support-ticket already verified it)
        const isGuestTicketCreation = normalizedType === "ticket_created" && !isAdmin && !requestingUserId;
        if (isGuestTicketCreation) {
            const createdAt = new Date(ticket.created_at).getTime();
            const now = Date.now();
            if (now - createdAt > 5 * 60 * 1000) { // 5 minutes
                logSupportDebug("ticket_created_notification_expired", {
                    ticket_id: String(ticket_id),
                    created_at: ticket.created_at,
                    now: new Date().toISOString(),
                });
                throw new Error("Forbidden: Ticket creation notification expired");
            }
            logSupportDebug("captcha_skipped_guest_verified_by_db", {
                ticket_id: String(ticket_id),
                type: normalizedType,
            });
