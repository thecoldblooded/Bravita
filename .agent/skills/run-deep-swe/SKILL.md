...
An OpenRouter key should be exported globally in `~/.zshrc` (a weekly hard spend limit is recommended as a safeguard). A fresh shell already has `OPENROUTER_API_KEY` available. If it's somehow not set, re-source the shell:
